...
const DIACRITICS = defaultDiacriticsRemovalMap.reduce((prev, cur) => {
...
const CLEANFCT = [
    { name: 'Trans Majuscule    ', op: value => !value ? value : value.toUpperCase() },
    { name: 'Suppr diacritiques ', op: value => !value ? value : value.replace(DIACRITICSRE, (m, p) => DIACRITICS[p], 'i') },
    { name: 'Suppr parenthese   ', op: value => !value ? value : value.replace(/^[^(]*\(/, ' ').replace(/\)[^)]*$/, ' ') },
    { name: 'Suppr liaisons     ', op: value => !value ? value : value.replace(SUPPRRE, '', 'gi') },
    { name: 'Abbrev type voie   ', op: value => !value ? value : value.replace(ABBREVRE, (m, p) => ABBREV[p], 'gi') },
    { name: 'Suppr no alphanum  ', op: value => !value ? value : value.replace(ALPHANUMRE, ' ') },
];
const CLEANWD = [
    { name: 'Trans Majuscule    ', op: value => !value ? value : value.toUpperCase() },
    { name: 'Suppr diacritiques ', op: value => !value ? value : value.replace(DIACRITICSRE, (m, p) => DIACRITICS[p], 'i') },
    ...
    { name: 'Abbrev type voie   ', op: value => !value ? value : value.replace(ABBREVRE, (m, p) => ABBREV[p], 'gi') },
    { name: 'Suppr no alphanum  ', op: value => !value ? value : value.replace(ALPHANUMRE, ' ') },
    { name: 'Suppr exces blancs ', op: value => !value ? value : value.replace(/^ */, '').replace(/ *$/g, '').split(/ +/) },
];
const CLEANWORDS = [
    { name: 'Trans Majuscule    ', op: value => !value ? value : value.toUpperCase() },
    { name: 'Suppr diacritiques ', op: value => !value ? value : value.replace(DIACRITICSRE, (m, p) => DIACRITICS[p], 'i') },
    { name: 'Suppr liaisons     ', op: value => !value ? value : value.replace(SUPPRRE, ' ', 'gi') },
    { name: 'Suppr no alphanum  ', op: value => !value ? value : value.replace(ALPHANUMRE, ' ') },
    { name: 'Suppr exces blancs ', op: value => !value ? value : value.replace(/^ */, '').replace(/ *$/g, '').replace(/ +/g, ' ') },
];
declare global {
    interface String {
        levenshtein(astring: string): number;
        clean(): string;
        wordList(): string[];
        cleanWords(): string;
        cleanPrefix(): string[];
...
String.prototype.levenshtein = function(astr: string): number {
    const [m, n, d] = [this.length, astr.length, []];
    if (!m) { return n; }
    if (!n) { return m; }
...
String.prototype.clean = function(): string {
    const cleaned = this ? CLEANFCT.reduce((v, fct) => fct.op(v), this) : '';
    return cleaned.replace(/  */g, ' ').replace(/^ */, '').replace(/ *$/g, '');
};

String.prototype.wordList = function(): string[] {
    return this ? CLEANWD.reduce((v, fct) => fct.op(v), this) : [];
};

String.prototype.cleanWords = function(): string {
    const cleaned = this ? CLEANWORDS.reduce((v, fct) => fct.op(v), this) : '';
    return cleaned;
};

String.prototype.cleanPrefix = function(this: string): string[] {
    return this ? this.cleanWords().split(/ +/) : [];
};

String.prototype.fuzzyHash = function(): number {
    const cleaned = this ? this.clean(this) : '';
    // all bits set to '0' for each ALPHANUMERIC
    const bits =  new Array(ALPHANUM.length).map(_ => '0');
    // one bit set to '1' for each ALPHANUMERIC char found
...
String.prototype.fuzzyExtend = (fuzzyh: number): number[] => {
    const values = [];
    const hibits = Math.floor(fuzzyh / POW_2_30);
    const lobits = fuzzyh % POW_2_30;
    for (let i = 0; i < ALPHANUM.length; i++) {
        const pow2i = Math.pow(2, i);
        const ihibits = Math.floor(pow2i / POW_2_30);
        const ilobits = pow2i % POW_2_30;
